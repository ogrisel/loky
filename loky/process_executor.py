...
def _python_exit():
    ...
    for t, wakeup in items:
        if t.is_alive():
            mp.util.debug('waking up thread "%s" on python exit' % t.name)
            wakeup.set()
    for t, _ in items:
        t.join()
...
def _queue_management_loop(executor_reference, processes, pending_work_items,
                           work_ids_queue, call_queue, result_queue, wakeup,
    ...
    Args:
        ...
        result_queue: A multiprocessing.Queue of _ResultItems generated by the
            process workers.
        wakeup: a sentinel to tell the thread to exit the result wait loop.
    """
    executor = None
    running_work_items = []
    ...
    while True:
        ...
        #   - The executor that owns this thread has been collected OR
        #   - The executor that owns this thread has been shutdown.
        if shutting_down() and len(pending_work_items) == 0:
            return
        else:
            _add_call_item_to_queue(pending_work_items, running_work_items,
                                    work_ids_queue, call_queue)
        if running_work_items and not shutting_down() and executor is not None:
            if len(executor._processes) != executor._max_workers:
                executor._start_missing_workers()
        executor = None
...
def _workers_management_loop(executor_reference, queue_management_thread,
                             processes, pending_work_items, work_ids,
                             call_queue, result_queue, kill_on_shutdown):
    ...
    def kill_all_workers():
        ...
        while processes:
            ...
            p.terminate()
            p.join()
        mp.util.debug('workers shutdown complete')

    def shutdown_all_workers():
        # Sent sentinel to workers and wait until they quit by them-selves
        mp.util.debug('waiting for shutdown of worker processes')
        nb_children_alive = None
        while nb_children_alive is None or nb_children_alive > 0:
            nb_children_alive = sum(
            ...
            try:
                ...
                # AssertionError can be raised if the queue has been closed
                # concurrently because of a crash for instance.
                break
        # Release the queue's resources as soon as possible.
        call_queue.close()
        ...
        if check_stopped_workers:
            ...
            for stopped_worker in stopped_workers:
                if stopped_worker.exitcode == WORKER_TIMEOUT_EXIT_CODE:
                    # Workers stopped by timeout have a specific return code:
                    # they should be collected but the executor should not be
                    # marked as broken
...
class ProcessPoolExecutor(_base.Executor):
    ...
    def _start_thread_management_thread(self):
        ...
        def weakref_cb(_, wakeup=self._queue_management_wakeup):
            mp.util.debug('waking up the queue manager thread')
            wakeup.set()
        if self._management_thread is None:
            mp.util.debug('starting workers management thread')
            # Start the processes so that their sentinels are known.
            self._workers_management_thread = threading.Thread(
                target=_workers_management_loop,
                ...
                name="ThreadManager")
            self._workers_management_thread.daemon = True
            self._workers_management_thread.start()
            _threads_wakeup[
    ...
    def submit(self, fn, *args, **kwargs):
        with self._shutdown_lock:
            ...
            self._queue_count += 1
            # # Wake up queue management thread so that
    ...
    def shutdown(self, wait=True):
        ...
        if self._workers_management_thread is not None:
            if wait and self._workers_management_thread.is_alive():
                self._workers_management_thread.join()
        ...
        # Make sure to release all Python objects that own refernces to file
        # descriptors.
        self._queue_management_thread = None
        self._workers_management_thread = None
